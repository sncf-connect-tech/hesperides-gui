...
technoModule.factory('TechnoService',
    ...
    return {
        get_model: function (name, version, isWorkingCopy){
            return $http.get(ENDPOINT + '/rest/templates/packages/' + encodeURIComponent(name) + '/' + encodeURIComponent(version) + '/' + (isWorkingCopy ? "workingcopy" : "release") + '/model').then(function(response){
        ...
        get_template_from_workingcopy: function (wc_name, wc_version, template_name) {
            return $http.get(ENDPOINT + '/rest/templates/packages/' + encodeURIComponent(wc_name) + '/' + encodeURIComponent(wc_version) + '/workingcopy/templates/' + encodeURIComponent(template_name)).then(function (response) {
        ...
        get_template_from_release: function (wc_name, wc_version, template_name) {
            return $http.get(ENDPOINT + '/rest/templates/packages/' + encodeURIComponent(wc_name) + '/' + encodeURIComponent(wc_version) + '/release/templates/' + encodeURIComponent(template_name)).then(function (response) {
                return new Template(response.data);
            }, function (error) {
                $.notify(error.data.message, "error");
                throw error;
        ...
        get_all_templates_from_workingcopy: function (wc_name, wc_version) {
            var baseUrl = ENDPOINT + '/rest/templates/packages/' + encodeURIComponent(wc_name) + '/' + encodeURIComponent(wc_version) + '/workingcopy/templates';
        ...
        get_all_templates_from_release: function (r_name, r_version) {
            return $http.get(ENDPOINT + '/rest/templates/packages/' + encodeURIComponent(r_name) + '/' + encodeURIComponent(r_version) + '/release/templates').then(function (response) {
                return response.data.map(function (data) {
                    var entry = new TemplateEntry(data);
                    var url = ENDPOINT + '/rest/templates/packages/' + encodeURIComponent(r_name) + '/' + encodeURIComponent(r_version) + '/release/templates/' + encodeURIComponent(entry.name);
                    entry.getTemplate(url).then (function (template){
                        entry.rights = FileService.files_rights_to_string(template.rights);
                        if (entry.rights < 0)
                            $translate('template.rights.none').then(function (label) { entry.rights = label; });
        ...
        save_template_in_workingcopy: function (wc_name, wc_version, template) {
            ...
            if (template.version_id < 0) {
                return $http.post(ENDPOINT + '/rest/templates/packages/' + encodeURIComponent(wc_name) + '/' + encodeURIComponent(wc_version) + '/workingcopy/templates', template).then(function (response) {
                ...
                return $http.put(ENDPOINT + '/rest/templates/packages/' + encodeURIComponent(wc_name) + '/' + encodeURIComponent(wc_version) + '/workingcopy/templates', template).then(function (response) {
        ...
        delete_template_in_workingcopy: function (wc_name, wc_version, template_name) {
            return $http.delete(ENDPOINT + '/rest/templates/packages/' + encodeURIComponent(wc_name) + '/' + encodeURIComponent(wc_version) + '/workingcopy/templates/' + encodeURIComponent(template_name)).then(function (response) {
        ...
        create_release: function (r_name, r_version) {
            return $http.post(ENDPOINT + '/rest/templates/packages/create_release?package_name=' + encodeURIComponent(r_name) + '&package_version=' + encodeURIComponent(r_version)).then(function (response) {
        ...
        create_workingcopy: function (wc_name, wc_version, from_name, from_version, is_from_workingcopy) {
            return $http.post(ENDPOINT + '/rest/templates/packages?from_package_name=' + encodeURIComponent(from_name) + '&from_package_version=' + encodeURIComponent(from_version) + '&from_is_working_copy=' + is_from_workingcopy, {name:encodeURIComponent(wc_name), version: encodeURIComponent(wc_version), working_copy:true}).then(function (response) {
                if (response.status === 201) {
        ...
        with_name_like: function (name) {
            if (!_.isUndefined(name)){
                if(name.length > 2) { //prevent search with too few characters
                    return $http.post(ENDPOINT + '/rest/templates/packages/perform_search?terms=' + encodeURIComponent(name.replace(' ', '#').replace('-', '#'))).then(function (response) {
